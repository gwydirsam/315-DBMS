...
class Grammar : public boost::spirit::qi::grammar<It, Program(), Skipper> {
 public:
  Grammar() : Grammar::base_type(program) {
    ...
    // // condition := conjunction { || conjunction }
    // conditions = (conjunctions >> *("||" >> conjunctions))[_val = boost::phoenix::construct<Condition>("OR", _1, _2)];
    // // conjunction := comparison { && comparison }
    // conjunctions = (comparisons >> *("&&" >> comparisons))[_val = boost::phoenix::construct<Condition>("AND", _1, _2)];
    // // comparison := operand op operand | ( condition )
    ...
    // // op := == | != | < | > | <= | >=
    // op = hold[string("||")] | hold[string("&&")] | hold[string("==")] | hold[string("!=")] | hold[string("<")] |
    ...
    atomic_expression =
        hold[("(" >> expression >> ")")] | relation_name;

    atomic_expressions =
        hold[("(" >> expressions >> ")")] | relation_name;
    ...
    selection =
        (no_case[string("select")] >> ("(" >> condition >> ")") >>
         atomic_expressions)[_val =
                            boost::phoenix::construct<Expression>(_1, _2, _3)];

    // projection := project ( attribute-list ) atomic-expr
    projection = (no_case[string("project")] >> ("(" >> attribute_list >>
                                                 ")") >> atomic_expressions)[_val = boost::phoenix::construct<Expression>(_1, _2, _3)];
    ...
    // setunion := atomic-expr + atomic-expr
    setunion = (atomic_expressions >> '+' >> atomic_expressions)[_val = boost::phoenix::construct<Expression>("union",_1,_2)];
    // difference := atomic-expr - atomic-expr
    difference = (atomic_expressions >> '-' >> atomic_expressions)[_val = boost::phoenix::construct<Expression>("difference",_1,_2)];
    // product := atomic-expr * atomic-expr
    product = (atomic_expressions >> '*' >> atomic_expressions)[_val = boost::phoenix::construct<Expression>("product",_1,_2)];
    ...
    expressions = (hold[selection] | hold[projection] | hold[renaming] | hold[setunion] |
